# Sprint 3 - Titel på sprinten

## Sprintperiod

**Start:** 2026-06-12  
**Slut:** 2026-06-13 
**Sprintlängd:** 2 dagar  

---

## Sprintmål

Skriv kort vad sprinten ska uppnå.

---

## Varför är sprinten värdefull?

Beskriv varför just denna sprint är viktig för produkten.

---

## Valda Issues för sprinten

| Issue | Titel | Ansvarig | Status |
|---|---|---|---|
| # | Titel | Namn | To Do |
| # | Titel | Namn | To Do |
| # | Titel | Namn | To Do |

---

## Plan för sprinten

- Gå igenom vilka issues som ingår i sprinten
- Säkerställa att alla issues har tydlig beskrivning och "Klart när"
- Dela upp för stora issues vid behov
- Assigna ansvariga personer
- Flytta kort i GitHub Projects när arbetet påbörjas
- Testa funktioner i rätt miljö
- Dokumentera resultatet

---

## Definition of Done

En issue räknas som klar när:

- Arbetet i issuen är färdigt
- Funktionen eller dokumentationen är testad
- Eventuella fel hanteras på ett rimligt sätt
- Resultatet är dokumenterat kort
- Kod eller dokumentation är pushad till GitHub
- Pull request är granskad eller godkänd av Product Owner
- Kortet är flyttat till Done i GitHub Projects

---

## Förändringsledning

**Finns det något i denna sprint som påverkar HR:s Awareness eller Desire kring onboarding-processen?**

Skriv kort om sprinten påverkar hur HR förstår behovet av förändringen, eller om något gör det lättare för HR att vilja delta i kravställningen.

**Svar:**

* Ingen särskild påverkan i denna sprint.


---

# Sammanfattning av sprinten

## Kort sammanfattning

Skriv 3-5 meningar om sprinten.

---

## Status efter sprinten

| Område | Status | Kommentar |
|---|---|---|
| GitHub Projects | Påbörjad/Klar |  |
| JSON/CSV | Påbörjad/Klar |  |
| PowerShell-script | Påbörjad/Klar |  |
| AD-testmiljö | Påbörjad/Klar |  |
| Dokumentation | Påbörjad/Klar |  |
| Förändringsledning | Påbörjad/Klar |  |

---

## Nästa sprint bör fokusera på

- 
- 
- 